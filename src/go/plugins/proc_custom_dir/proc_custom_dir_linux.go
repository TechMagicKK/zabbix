...
type PluginOptions struct {
	ProcDirName string `conf:"optional,default=/proc"`
}
...
func (p *Plugin) Configure(global *plugin.GlobalOptions, privateOptions interface{}) {
	...
	if p.options.ProcDirName == "" {
		p.options.ProcDirName = "/proc"
	}
}

func (p *Plugin) Validate(privateOptions interface{}) error {
	var opts PluginOptions
	if err := conf.Unmarshal(privateOptions, &opts); err != nil {
		return fmt.Errorf("cannot unmarshal configuration options for validation: %s", err)
	}

	if opts.ProcDirName == "" {
		return fmt.Errorf("ProcDirName must not be empty")
	}

	if strings.Contains(opts.ProcDirName, "./") {
		return fmt.Errorf("ProcDirName must be in absolute path: %s", opts.ProcDirName)
	}

	if strings.HasPrefix(opts.ProcDirName, "/") == false {
		return fmt.Errorf("ProcDirName must be from root path: %s", opts.ProcDirName)
	}
...
func (p *Plugin) Collect() (err error) {
	...
	if processes, err = getProcesses(p.options.ProcDirName, flags); err != nil {
		return
	}
	...
	for pid, stat := range stats {
		p.getProcCpuUtil(p.options.ProcDirName, pid, stat)
		if stat.err != nil {
...
func (p *PluginExport) Configure(global *plugin.GlobalOptions, privateOptions interface{}) {
	if err := conf.Unmarshal(privateOptions, &p.options); err != nil {
		p.Errf("cannot unmarshal configuration options: %s", err)
	}
...
func (p *PluginExport) Validate(privateOptions interface{}) error {
	var opts PluginOptions
	if err := conf.Unmarshal(privateOptions, &opts); err != nil {
		return fmt.Errorf("cannot unmarshal configuration options for validation: %s", err)
	}
...
func (p *PluginExport) exportProcMem(params []string) (result interface{}, err error) {
	...
	var mem uint64
	if memtype == "pmem" {
		mem, err = procfs_custom_dir.GetMemory(p.options.ProcDirName, "MemTotal")
		if err != nil {
	...
	processes, err := getProcesses(p.options.ProcDirName, procInfoName|procInfoCmdline|procInfoUser)
	if err != nil {
	...
	for _, proc := range processes {
		...
		data, err := procfs_custom_dir.ReadAll(fmt.Sprintf("/%s/%s/status", p.options.ProcDirName, strconv.FormatInt(proc.pid, 10)))
		if err != nil {
		...
		switch memtype {
		case "pmem":
			vmRSS, found, err := procfs_custom_dir.ByteFromProcFileData(data, "VmRSS")
			if err != nil {
		...
		case "size":
			vmData, found, err := procfs_custom_dir.ByteFromProcFileData(data, "VmData")
			if err != nil {
			...
			vmStk, found, err := procfs_custom_dir.ByteFromProcFileData(data, "VmStk")
			if err != nil {
			...
			vmExe, found, err := procfs_custom_dir.ByteFromProcFileData(data, "VmExe")
			if err != nil {
		...
		default:
			typeValue, found, err := procfs_custom_dir.ByteFromProcFileData(data, typeStr)
			if err != nil {
...
func (p *PluginExport) exportProcNum(params []string) (interface{}, error) {
	...
	procs, err := getProcesses(p.options.ProcDirName, flags)
	if err != nil {
...
func init() {
	plugin.RegisterMetrics(&impl, "ProcCustom", "proccustom.cpu.util", "Process CPU utilization percentage.")
	plugin.RegisterMetrics(&implExport, "ProcCustomExporter",
		"proccustom.mem", "Process memory utilization values.",
		"proccustom.num", "The number of processes.",
	)
}
